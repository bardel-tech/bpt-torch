...
class TestMetric(unittest.TestCase):
    def test_normalize_labels(self):
        self.assertEquals(
            frozenset([("foo", "bar")]),
            Metric.normalize_labels(frozenset([("foo", "bar")])),
        )
        self.assertEquals(frozenset(), Metric.normalize_labels(None))
        self.assertEquals(
            frozenset([("foo", "bar")]), Metric.normalize_labels({"foo": "bar"})
        )
        self.assertEquals(
            frozenset([("foo", "bar")]), Metric.normalize_labels([("foo", "bar")])
        )
    ...
    def test_render_part(self):
        ...
        self.assertEquals(
            "foo 5.0",
            metric_no_labels.render_part(
                metric_no_labels.name, metric_no_labels.labels, 5.0
            ),
        )

        metric = Metric("foo1", {"bar": "baz"})
        self.assertEquals(
            'foo1{bar="baz"} 5.0', metric.render_part(metric.name, metric.labels, 5.0)
        )
...
class TestCounter(unittest.TestCase):
    ...
    def test_render(self):
        self.counter.inc()
        self.assertEquals('c{foo="bar"} 1.0', self.counter.render())
...
class TestGauge(unittest.TestCase):
    ...
    def test_render(self):
        self.gauge.inc()
        self.assertEquals('g{foo="bar"} 1.0', self.gauge.render())
...
class TestSummary(unittest.TestCase):
    ...
    def test_render(self):
        ...
        self.assertEquals(
            's_count{foo="bar"} 1.0\n' 's_sum{foo="bar"} 1.0', self.summary.render()
        )
...
class TestHistogram(unittest.TestCase):
    ...
    def test_setting_buckets(self):
        ...
        self.assertRaises(
            ValueError, Histogram, "h", {"foo": "bar"}, buckets=[float("inf")]
        )
