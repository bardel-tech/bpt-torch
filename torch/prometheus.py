...
class Metric(object):
    ...
    def format_labels(label_pairs):
        label_pairs = ",".join(
            ['{}="{}"'.format(key, value) for key, value in sorted(label_pairs)]
        )
        if label_pairs:
...
class Histogram(Metric):
    ...
    def render(self):
        ...
        for bucket, value in self.buckets.iteritems():
            metrics.append(
                self.render_part(
                    bucket_name, bucket_labels.union([("le", bucket)]), value
                )
            )
        return "\n".join(metrics)
...
class Registry(OrderedDict):
    ...
    def add_metric(self, klass, name, description, **kwargs):
        return self.setdefault(
            (klass, name), MetricFamily(klass, name, description, **kwargs)
        )
